...
//			Telemetry.sendMetric("5", getTimer() );
            mStage.render(mSupport, 1.0);	//慢
//			Telemetry.sendMetric("6", getTimer() );
            mSupport.finishQuadBatch();
            
            if (mStatsDisplay)
                mStatsDisplay.drawCount = mSupport.drawCount;
            
//			Telemetry.sendMetric("8", getTimer()-kStart );
			
			//慢
            if (!mShareContext)
                mContext.present();
			
//			Telemetry.sendMetric("9", getTimer()-kStart );
        }
        ...
        public function showStatsAt(hAlign:String="left", vAlign:String="top"):void
        {
            ...
            else
            {
                if (mStatsDisplay == null)
                {
					...
					mStatsDisplay.scaleX = mStatsDisplay.scaleY = 3;
